...
static void childProcess(const GraphData *data, int travelerIndex, int writeFd) {
    ...
    for (int i = 0; i < path.pathLength; i++) {
        ...
        if (i + 1 < path.pathLength - 1) usleep(WAIT_TIME_US);
    }
...
int main(int argc, char *argv[]) {
    ...
    InitWindow(SCREEN_WIDTH, SCREEN_HEIGHT, "Milestone 5 - IPC with Pipes");
    SetTargetFPS(60);
    ...
    while (!WindowShouldClose()) {
        ...
        DrawText("Milestone 5 - IPC with Pipes", 30, 20, 28, DARKBLUE);
        DrawText("Children compute Dijkstra and send node updates. Parent prints logs and updates GUI.", 30, 60, 20, DARKGRAY);
        ...
        for (int i = 0; i < data.travelerCount; i++) {
            ...
            DrawText(label, (int)views[i].position.x + 14, (int)views[i].position.y - 10, 18, views[i].color);
        }

        if (finishedCount == data.travelerCount) DrawText("All travelers finished.", 30, 95, 24, GREEN);
        EndDrawing();
    }
    ...
    for (int i = 0; i < data.travelerCount; i++) waitpid(views[i].pid, NULL, 0);
    return 0;
}
